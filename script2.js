...
// Function to check role restrictions
/* function checkRoleRestrictions() {
    const selectedRoleCount = roleCounts[player.role];
   switch(player.role){
    case 'wk':
        if (selectedRoleCount >defauldObject.ar.minimum) {
...
function validateCricketTeam(selectedPlayers) {
    // Define the required player types and their allowed count range
    const playerTypes = {
      batsman: { min: 3, max: 6 },
      bowler: { min: 3, max: 6 },
    ...
    for (const player of selectedPlayers) {
      ...
      if (playerTypes[type] === undefined) {
        return "Invalid player type.";
      }
  
      // Check if the player count for the player type exceeds the maximum limit
      if (playerCounts[type] >= playerTypes[type].max) {
        return `Maximum ${type}s limit reached.`;
      }
  
      // Increment the player count for the player type
      playerCounts[type]++;
    }
  
    // Check if the player count for the player type is below the minimum limit
    for (const type in playerTypes) {
      if (playerCounts[type] < playerTypes[type].min) {
        return `Minimum ${playerTypes[type].min} ${type}s required.`;
      }
    }
