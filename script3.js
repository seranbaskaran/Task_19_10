...
let roleCounts = {
    wk: 0,bat: 0,ar: 0,bowl: 0,teamAct:0,teamBct:0,totalCount:0
};
function checkCount() {
...
/* function validateTeam(allPlayer){
  
  const selectedRoleCount=roleCounts['wk'];
  switch('wk'){
    case 'wk':
      if(selectedRoleCount>defaultObject.wk.min){
        let count=selectedRoleCount+defaultObject.bat.min+defaultObject.ar.min+defaultObject.bowl.min;
        if(count>11){
          for(let x in defaultObject){
           defaultObject[x][max]--;
           console.log(defaultObject[x][max]);
          }
        }
      }
  }
...
/* function validateTeam(allPlayer) {
  ...
  // Count the selected players for each role
  allPlayer.forEach((player) => {
    if (player.visible) {
      selectedRoleCount[player.role]++;
    }
  });

  // Check and adjust the team selection
  if (selectedRoleCount.wk > defaultObject.wk.max) {
    // Reduce the maximum count of wicket-keepers
    defaultObject.wk.max--;

    // Ensure that the total player count is not more than 11
    let totalSelectedCount = selectedRoleCount.wk + selectedRoleCount.bat + selectedRoleCount.ar + selectedRoleCount.bowl;
    while (totalSelectedCount > 11) {
      // Try to decrease other player types' max limits
      if (selectedRoleCount.bat > defaultObject.bat.min) {
        defaultObject.bat.max--;
      } else if (selectedRoleCount.ar > defaultObject.ar.min) {
        defaultObject.ar.max--;
      } else if (selectedRoleCount.bowl > defaultObject.bowl.min) {
        defaultObject.bowl.max--;
      }
      // Update the total selected count
      totalSelectedCount = selectedRoleCount.wk + defaultObject.bat.max + defaultObject.ar.max + defaultObject.bowl.max;
    }
  }

  // Apply the updated limits to the playerTypes object
  playerTypes.bat.min = defaultObject.bat.min;
  playerTypes.bat.max = defaultObject.bat.max;
  playerTypes.ar.min = defaultObject.ar.min;
  playerTypes.ar.max = defaultObject.ar.max;
  playerTypes.wk.min = defaultObject.wk.min;
  playerTypes.wk.max = defaultObject.wk.max;
  playerTypes.bowl.min = defaultObject.bowl.min;
  playerTypes.bowl.max = defaultObject.bowl.max;
} */
...
function validateTeam(allPlayer) {
  ...
  // Check and adjust the team selection for each role
  for (const role in selectedRoleCount) {
    const roleCount = selectedRoleCount[role];
    const roleLimits = defaultObject[role];
  
    if (roleCount > roleLimits.max) {
      // Reduce the maximum count for the current role
      roleLimits.max--;

      // Ensure that the total player count is not more than 11
      let totalSelectedCount = 0;
      for (const r in selectedRoleCount) {
        totalSelectedCount += selectedRoleCount[r];
      }

      // Try to decrease other player types' max limits
      while (totalSelectedCount > 11) {
        for (const r in selectedRoleCount) {
          if (selectedRoleCount[r] > defaultObject[r].min) {
            defaultObject[r].max--;
            totalSelectedCount--;
          }
        }
    ...
    // Update the role-specific limits
    playerTypes[role].min = defaultObject[role].min;
    playerTypes[role].max = defaultObject[role].max;
  }
}
